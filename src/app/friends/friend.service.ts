...
export class FriendService {
  ...
  constructor() {
    ...
    from(this.db.persons.toArray()).subscribe(
      data => { // create initial graph
  ...
  addPerson(person: Person): Observable<string> {
    if (person.name in this.persons) {
      return throwError(new Error(`A person with name ${person.name} already exists`));
    }
    person = { ...person }; // make a copy
    return from(this.db.persons.add(person)).pipe(
      mergeMap(() => { // add new person to in-memory store
        ...
        const updates: Person[] = person.friends.map(f => ({...this.persons[f]}));
        return from(this.db.persons.bulkPut(updates));
      }),
      mergeMap(() => { // emit new state
